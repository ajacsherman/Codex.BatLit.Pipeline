...
A later Zotero test showed that the Zotero `Metadata Retrieval` window can still open when PDFs are attached during import. That process is Zotero's own PDF-text lookup and can create parent items with partial guessed metadata, even when the RIS and embedded PDF metadata are richer. To prevent this, the package builder now also writes a `_metadata_only.ris` file with no `L1` PDF attachment links. If Zotero attempts metadata retrieval during import, the safer workflow is to import `_metadata_only.ris` first, then attach PDFs to the already-created parent items afterward.

The package builder now also writes `_metadata_only.csl.json`. CSL JSON is a Zotero-supported bibliographic import format and is useful for BatLit-curated citations that are not reliably discoverable online. This makes the workflow citation-first: BatLit can supply a structured title, creators, year, journal or source-work title, volume, issue, pages, ISSN/DOI/URL, abstract, and provenance note without requiring Zotero to find a matching online record. PDF attachment becomes a separate step after parent citation records are safely created.

## Online Availability Audit

The AMNH test batch exposed a missing gate in the workflow. Before any AMNH or taxonomic-history scan is treated as unique, Zotero-ready, or final new literature, the pipeline must run an online availability audit. This audit starts from citation evidence rather than PDF metadata and records whether the item is already in BatLit/Zotero/Zenodo, has a JSTOR stable record, has a BHL/BioStor/Internet Archive scan candidate, has online metadata or identifiers, has only low-confidence API candidates, or still requires manual review.

The audit deliberately separates online evidence from API guesses. Crossref, OpenAlex, and Internet Archive results are scored by normalized title similarity before they are accepted as evidence. Low-similarity results are retained as review candidates but are not used to conclude that a paper is online or has a valid DOI. JSTOR and Google Scholar are represented as exact-title review links because they are not stable public-harvest APIs for this workflow.

For the AMNH 2026-08-27 batch, the corrected API-scored audit was written to `metadata_enrichment/AMNH_20260827/AMNH_20260827_online_availability_audit_with_apis.csv`. The current decision summary is: one `jstor_record_found`, three `online_metadata_or_identifier_found`, three `online_scan_candidate_found`, one `already_in_batlit`, and twelve `api_candidates_need_review`. The Lazell and Koopman 1985 paper is now explicitly recorded as `jstor_record_found`, and Dobson 1878 is recorded as `already_in_batlit`.

No future AMNH scan should be called new literature or Zotero-ready until this audit has been completed and the `amnh_scan_value`/review notes explain whether the scan is redundant, better quality, an excerpt, annotated, or still unresolved.
